...
from openpilot.tools.hrr_updater.protocol import UpdateState
...
class FakeHrrTransport(CanTransport):
  """In-memory application/bootloader with durable state and ACK fault injection."""

  def __init__(self, bus: int = 2, *, active_slot: int = SLOT_A,
  ...
  def _handle(self, payload: bytes) -> None:
    ...
    if service == SVC_GET_INFO:
      ...
      if (self.state in (UpdateState.PROGRAMMING, UpdateState.PROGRAMMED) and token == self.token and
      ...
      self.state, self.error = UpdateState.FAILED, 0
      self._single(self._status_base(service), service)
