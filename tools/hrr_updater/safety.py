...
from openpilot.tools.hrr_updater.errors import UnsafeStateError
...
class OpenpilotSafetyGate:
  ...
  def __init__(self, bus: int):
    ...
    self.acquired = False
    self.in_progress = self.params.get_bool("HrrUpdateInProgress")
  ...
  def check(self) -> None:
    ...
    if ignition and not self.params.get_bool("HrrUpdateStartupHold"):
      raise UnsafeStateError("ignition is active without an HRR pre-ONROAD startup hold")
    if panda.controlsAllowed:
      raise UnsafeStateError("Panda controls are allowed")

  def acquire(self, timeout: float = 10.0) -> None:
    self.check()
    self.params.put_bool("HrrUpdaterRequested", True)
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
      self._update(250)
      panda = self._panda()
      if (self.params.get_bool("HrrUpdaterReady") and panda.safetyModel.raw == self.SAFETY_HRR_UPDATER and
          not panda.powerSaveEnabled):
        self.acquired = True
        return
    self.params.put_bool("HrrUpdaterRequested", False)
    raise UnsafeStateError("pandad did not grant the HRR pre-ONROAD update lease")

  def mark_in_progress(self) -> None:
    self.check()
    if not self.acquired:
      raise UnsafeStateError("HRR update lease is not active")
    self.params.put("HrrUpdateDeadline", str(int(time.time()) + self.UPDATE_DEADLINE_SECONDS))
    self.params.put_bool("HrrUpdateInProgress", True)
    self.in_progress = True

  def check_during_update(self) -> None:
    self._update(0)
    if self.sm["deviceState"].started or self.params.get_bool("IsOnroad") or self.params.get_bool("IsEngaged"):
      raise UnsafeStateError("openpilot entered an unsafe state during the HRR update")
    panda = self._panda()
    if (panda.ignitionLine or panda.ignitionCan) and not self.params.get_bool("HrrUpdateStartupHold"):
      raise UnsafeStateError("HRR startup hold was released during the update")
    if panda.safetyModel.raw != self.SAFETY_HRR_UPDATER or panda.powerSaveEnabled:
      raise UnsafeStateError("Panda left HRR updater safety mode")
  ...
  def release(self, *, preserve_in_progress: bool = False) -> None:
    if not preserve_in_progress:
      self.params.put_bool("HrrUpdateInProgress", False)
      self.params.remove("HrrUpdateDeadline")
      self.params.put_bool("HrrUpdateStartupHold", False)
    self.params.put_bool("HrrUpdaterRequested", False)
    self.params.put_bool("HrrUpdaterReady", False)
