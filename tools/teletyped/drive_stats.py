...
class VehicleStatsProfile:
  ...
  name = "unknown"
  supports_steering_interventions = False
...
class HyundaiI30StatsProfile(VehicleStatsProfile):
  ...
  supports_steering_interventions = True
  fingerprints = {
    "HYUNDAI I30 GD 2014",
    "HYUNDAI_I30_GD_2014",
  }
...
class LexusLS600hStatsProfile(VehicleStatsProfile):
  name = "lexus_ls600h_2013_17"
  steering_mode = "hrr_torque"
  fingerprints = {
    "LEXUS_LS600H",
    "LEXUS LS 600H 2013-17",
  }

  def matches(self, car_name: str, fingerprint: str) -> bool:
    return car_name.strip().lower() == "toyota" and fingerprint.strip().upper() in self.fingerprints


VEHICLE_PROFILES: tuple[VehicleStatsProfile, ...] = (HyundaiI30StatsProfile(), LexusLS600hStatsProfile())
...
class DriveAnalyzer:
  ...
  def _observe_torque(self, now_ns: int, car_state: Any) -> None:
    if self.profile is None or not self.profile.supports_steering_interventions:
      return
  ...
  def result(self) -> dict[str, Any] | None:
    ...
    steering_interventions_available = self.profile.supports_steering_interventions
    uses_pressed_interventions = steering_interventions_available and (
      self.trqi_steering is True or (self.trqi_steering is None and not self.saw_legacy_steering_feedback)
    )
    steering_interventions = (
      self.pressed_intervention_count if uses_pressed_interventions else self.torque_intervention_count
    ) if steering_interventions_available else None
    ...
    steer_per_100km = steering_interventions / total_distance * 100 if steering_interventions is not None and total_distance > 0 else None
    ...
    for definition in SPEED_BUCKETS:
      ...
      bucket_steering_interventions = (
        int(bucket["pressed_intervention_count"] if uses_pressed_interventions else bucket["torque_intervention_count"])
        if steering_interventions_available else None
      )
      speed_buckets[key] = {
        ...
        "steer_interventions_per_100km": (
          round(bucket_steering_interventions / distance_km * 100, 2)
          if bucket_steering_interventions is not None and distance_km > 0 else None
        ),
    ...
    return {
      ...
      "steer_intervention_source": (
        "steering_pressed" if uses_pressed_interventions else "torque_delta"
      ) if steering_interventions_available else None,
      ...
      "steering_mode": (
        "trqi_tq_delta" if self.trqi_steering else "legacy_ssc" if self.trqi_steering is False else "unknown"
      ) if steering_interventions_available else self.profile.steering_mode,
