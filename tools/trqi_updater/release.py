...
from openpilot.tools.trqi_updater.constants import PRODUCT_ID, SLOT_NAMES
from openpilot.tools.trqi_updater.errors import NetworkFailure, ReleaseVerificationError
...
def verify_release_signature(raw: bytes, signature: bytes, public_key: bytes) -> dict[str, Any]:
  ...
  if parsed.get("product") != "TRQI" or parsed.get("hardware_id") != "TRQI_G474_V1":
    raise ReleaseVerificationError("release metadata hardware identity mismatch")
  try:
    Version.parse(parsed["firmware_version"])
  except (KeyError, TypeError, ValueError) as exc:
    raise ReleaseVerificationError("release metadata firmware version is invalid") from exc
  if not isinstance(parsed.get("security_version"), int) or parsed["security_version"] < 0:
    raise ReleaseVerificationError("release metadata security version is invalid")
  if not isinstance(parsed.get("git_sha"), str) or GIT_SHA_PATTERN.fullmatch(parsed["git_sha"]) is None:
    raise ReleaseVerificationError("release metadata git SHA is invalid")
  ...
  try:
    Version.parse(parsed["minimum_updater_version"])
  except (KeyError, TypeError, ValueError) as exc:
    raise ReleaseVerificationError("release metadata minimum updater version is invalid") from exc
  ...
  for slot in ("A", "B"):
    ...
    if (not isinstance(entry["image"], str) or Path(entry["image"]).name != entry["image"] or
        not isinstance(entry["manifest"], str) or Path(entry["manifest"]).name != entry["manifest"]):
      raise ReleaseVerificationError(f"release metadata slot {slot} has an unsafe asset name")
    if not isinstance(entry["size"], int) or entry["size"] < 8:
      raise ReleaseVerificationError(f"release metadata slot {slot} has an invalid image size")
    if (not isinstance(entry["sha256"], str) or SHA256_PATTERN.fullmatch(entry["sha256"]) is None or
        not isinstance(entry["manifest_sha256"], str) or
        SHA256_PATTERN.fullmatch(entry["manifest_sha256"]) is None):
      raise ReleaseVerificationError(f"release metadata slot {slot} has an invalid SHA-256")
...
class GitHubReleaseClient:
  def __init__(self, repository: str = "killinen/TRQI", token: str | None = None, timeout: float = 30.0):
    self.repository = repository
    self.token = token or os.getenv("TRQI_GITHUB_TOKEN") or os.getenv("GH_TOKEN") or self._params_token()
    self.timeout = timeout

  @staticmethod
  def _params_token() -> str | None:
    # Managed processes do not normally inherit an interactive shell's token.
    # Keep the private-repository credential in a DONT_LOG Params key instead.
    try:
      from openpilot.common.params import Params
      return Params().get("TrqiGithubToken", encoding="utf8") or None
    except Exception:
      return None
