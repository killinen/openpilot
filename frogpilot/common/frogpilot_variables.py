...
frogpilot_default_params: list[tuple[str, str | bytes, int, str]] = [
  ...
  ("AlwaysOnLateralLKAS", "0", 2, "0"),
  ("AlwaysOnLateralMain", "0", 2, "0"),
  ("AMapKey1", "", 0, ""),
  ("AMapKey2", "", 0, ""),
...
class FrogPilotVariables:
  ...
  def update(self, holiday_theme, started):
    ...
    has_auto_tune = toggle.car_make in {"hyundai", "i30", "toyota"} and CP.lateralTuning.which() == "torque"
    ...
    steerKpParam = params.get_float("SteerKP")
    steerKpStock = params.get_float("SteerKPStock")
    toggle.use_custom_steerKp = advanced_lateral_tuning and is_torque_car and tuning_level >= level["SteerKP"] and bool(round(steerKpParam, 2) != round(steerKpStock, 2))
    toggle.steerKp = np.clip(steerKpParam, steerKp * 0.5, steerKp * 1.5) if toggle.use_custom_steerKp else steerKp
    toggle.steerKp = [[0], [toggle.steerKp]]
    ...
    toggle.no_logging = device_management and (params.get_bool("NoLogging") if tuning_level >= level["NoLogging"] else default.get_bool("NoLogging")) and not self.vetting_branch
