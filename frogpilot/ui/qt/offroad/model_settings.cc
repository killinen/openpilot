  ...
  for (const auto &[param, title, desc, icon] : modelToggles) {
    ...
    if (param == "DeleteModel") {
      ...
      QObject::connect(deleteModelButton, &FrogPilotButtonsControl::buttonClicked, [this](int id) {
        ...
        deletableModels.removeAll(modelFileToNameMapProcessed.value(normalizeModelKey(QString::fromStdString("firehose"))));
      ...
      QObject::connect(selectModelButton, &ButtonControl::clicked, [this]() {
        ...
        selectableModels.prepend(modelFileToNameMap.value(normalizeModelKey(QString::fromStdString("firehose"))));

        QString modelToSelect = MultiOptionDialog::getSelection(tr("Select a Model"), selectableModels, currentModel, this);
...
void FrogPilotModelPanel::showEvent(QShowEvent *event) {
  ...
  if (!modelDir.exists(modelKey + ".thneed") && !hasAllTinygradFiles(modelDir, modelKey)) {
    modelKey = normalizeModelKey(QString::fromStdString("firehose"));
  }
...
void FrogPilotModelPanel::updateState(const UIState &s, const FrogPilotUIState &fs) {
  ...
  bool parked = !started || frogpilot_scene.parked || frogpilot_scene.frogpilot_toggles.value("frogs_go_moo").toBool();
