...
void FrogPilotMapsPanel::showEvent(QShowEvent *event) {
  ...
  bool parked = !scene.started || frogpilot_scene.parked || frogpilot_scene.frogpilot_toggles.value("frogs_go_moo").toBool();
  ...
  if (!osmDownloadProgress.empty()) {
    ...
    downloadMapsButton->setValue(frogpilot_scene.online ? (parked ? "" : "Not parked") : tr("Offline..."));
  }
}
