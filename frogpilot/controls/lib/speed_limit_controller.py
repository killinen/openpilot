...
class SpeedLimitController:
  def __init__(self, params):
    ...
    self.mapbox_requests = params.get("MapBoxRequests")
    ...
    self.mapbox_token = params.get("MapboxSecretKey")

    self.previous_target = params.get("PreviousSpeedLimit")
  ...
  def get_mapbox_speed_limit(self, gps_position, now, time_validated, v_ego, params, sm):
    ...
    def make_request():
      ...
      try:
        ...
        params.put_nonblocking("MapBoxRequests", self.mapbox_requests)
  ...
  def handle_limit_change(self, desired_source, desired_target, params, params_memory, sm):
    ...
    if self.target != self.previous_target and self.target > 0 and not speed_limit_denied:
      ...
      params.put_nonblocking("PreviousSpeedLimit", self.target)

  def update_limits(self, dashboard_speed_limit, gps_position, now, time_validated, v_cruise, v_ego, params, params_memory, sm):
    self.update_map_speed_limit(gps_position, v_ego, params_memory)
    ...
    if desired_target == 0 or self.target == 0:
      if self.mapbox_requests["total_requests"] < self.mapbox_requests["max_requests"] and self.frogpilot_toggles.slc_mapbox_filler:
        self.get_mapbox_speed_limit(gps_position, now, time_validated, v_ego, params, sm)
    ...
    if abs(desired_target - self.previous_target) >= 1:
      self.handle_limit_change(desired_source, desired_target, params, params_memory, sm)
  ...
  def update_map_speed_limit(self, gps_position, v_ego, params_memory):
    if not gps_position:
      return

    self.map_speed_limit = params_memory.get("MapSpeedLimit")

    next_map_speed_limit = params_memory.get("NextMapSpeedLimit")
