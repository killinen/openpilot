...
def covert_audio(input_file):
  ...
  try:
    subprocess.run([
      ...
      "-ar", "48000", "-ac", "1", "-c:a", "pcm_s16le", "-y", str(output_filename)
    ], check=True)
  except subprocess.CalledProcessError as e:
    print(f"Failed to convert {input_file} to WAV: {e}")
    return

  if input_file != output_filename and os.path.exists(input_file):
...
def get_drive_stats():
  stats = json.loads(params.get("ApiCache_DriveStats") or "{}")
