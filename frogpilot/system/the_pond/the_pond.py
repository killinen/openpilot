...
def setup(app):
  ...
  def doors_available():
    with car.CarParams.from_bytes(utilities.params.get("CarParamsPersistent")) as cp_reader:
      CP = cp_reader.as_builder()

    return jsonify({"result": HARDWARE.get_device_type() != "tici" and CP.brand == "toyota"})
  ...
  def lock_doors():
    ...
    while True:
      with Panda(disable_checks=True) as panda:
        if not utilities.params.get_bool("IsOnroad"):
  ...
  def clear_navigation():
    utilities.params.remove("NavDestination")
    return {"message": "Destination cleared"}

  @app.route("/api/navigation", methods=["GET"])
  def navigation():
    last_position = utilities.params.get("LastGPSPosition") or "{\"latitude\": 51.276824158421331, \"longitude\": 30.221928335547232, \"altitude\": 111.0}"

    return {
      "amap1Key": utilities.params.get("AMapKey1") or "",
      "amap2Key": utilities.params.get("AMapKey2") or "",
      "destination": utilities.params.get("NavDestination") or "",
      "isMetric": utilities.params.get_bool("IsMetric"),
      "lastPosition": {
        "latitude": str(last_position["latitude"]),
        "longitude": str(last_position["longitude"])
      },
      "mapboxPublic": utilities.params.get("MapboxPublicKey") or "",
      "mapboxSecret": utilities.params.get("MapboxSecretKey") or "",
      "previousDestinations": utilities.params.get("ApiCache_NavDestinations") or "",
    }
  ...
  def set_navigation():
    ...
    utilities.params.put("NavDestination", request.json)
  ...
  def remove_favorite_destination():
    ...
    existing = utilities.params.get("FavoriteDestinations")
    ...
    utilities.params.put("FavoriteDestinations", favorites)
  ...
  def list_favorite_destinations():
    favorites = utilities.params.get("FavoriteDestinations")
    changed = False
    for f in favorites:
      if "id" not in f:
  ...
  def add_favorite_destination():
    ...
    utilities.params.put("FavoriteDestinations", existing)
  ...
  def rename_favorite_destination():
    ...
    existing_favorites = utilities.params.get("FavoriteDestinations")
    ...
    utilities.params.put("FavoriteDestinations",existing_favorites)
  ...
  def delete_navigation_key():
    ...
    utilities.params.remove(meta[2])
  ...
  def set_navigation_keys():
    ...
    for meta in KEYS.values():
      ...
      utilities.params.put(meta[2], full)
      saved.append(meta[3])
  ...
  def get_param():
    return utilities.params.get(request.args.get("key")) or "", 200
  ...
  def get_stats():
    ...
    return {
      ...
      "softwareInfo": {
        ...
        "updateAvailable": "Yes" if utilities.params.get_bool("UpdaterFetchAvailable") else "No",
  ...
  def apply_theme():
    ...
    utilities.params.put_bool("PersonalizeOpenpilot", True)
  ...
  def get_default_theme():
    ...
    if not utilities.params.get_bool("PersonalizeOpenpilot"):
      ...
      for param, theme_key in theme_param_map.items():
        param_value = utilities.params.get(param)
        if param_value:
  ...
  def delete_secoc_key():
    ...
    keys = utilities.params.get("SecOCKeys")
    keys = [key for key in keys if key.get("name") != name]
    utilities.params.put("SecOCKeys", keys)
    return jsonify(keys)

  @app.route("/api/tsk_keys", methods=["GET"])
  def get_secoc_keys():
    return jsonify(json.loads(utilities.params.get("SecOCKeys", "[]")))
  ...
  def set_secoc_key():
    ...
    utilities.params.put("SecOCKey", value)
  ...
  def backup_toggle_values():
    toggle_values = {}
    for key in utilities.params.all_keys():
      if key in EXCLUDED_KEYS:
        continue

      raw_value = utilities.params.get(key)
  ...
  def restore_toggle_values():
    ...
    all_param_keys = set(utilities.params.all_keys())
    allowed_keys = all_param_keys - set(EXCLUDED_KEYS)
    ...
    try:
      ...
      return jsonify({"success": False, "message": f"Failed to decode data: {e}"}), 400

    for key, value in toggle_values.items():
      if key in allowed_keys:
        try:
          utilities.params.put(key, value)
        except TypeError:
          print(f"Skipping restore for key '{key}': Invalid value type.")
          continue

    update_frogpilot_toggles()
    return jsonify({"success": True, "message": "Toggles restored successfully!"})

  @app.route("/api/toggles/reset_default", methods=["POST"])
  def reset_toggle_values():
    for k in utilities.params.all_keys():
      default_value = utilities.params.get_default_value(k)
      if default_value is not None:
        utilities.params.put(k, default_value)

  @app.route("/api/toggles/reset_stock", methods=["POST"])
  def reset_toggle_values_to_stock():
    for k in utilities.params.all_keys():
      stock_value = utilities.params.get_stock_value(k)
      if stock_value is not None:
        utilities.params.put(k, stock_value)
