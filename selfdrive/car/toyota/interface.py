...
from openpilot.selfdrive.car.toyota.values import Ecu, CAR, DBC, ToyotaFlags, CarControllerParams, TSS2_CAR, RADAR_ACC_CAR, NO_DSU_CAR, \
                                        MIN_ACC_SPEED, EPS_SCALE, UNSUPPORTED_DSU_CAR, NO_STOP_TIMER_CAR, ANGLE_CONTROL_CAR, STOP_AND_GO_CAR, HRR_CAR
...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret, candidate, fingerprint, car_fw, experimental_long, docs, frogpilot_toggles):
    ...
    if candidate in HRR_CAR:
      ret.safetyConfigs[0].safetyParam |= Panda.FLAG_TOYOTA_HRR
  ...
  def _update(self, c, frogpilot_toggles):
    ret, fp_ret = self.CS.update(self.cp, self.cp_cam, c, frogpilot_toggles, self.cp_body)
    ...
    if self.CP.carFingerprint in HRR_CAR and self.CS.ls600h_hrr_ecu_overtemperature:
      events.add(EventName.hrrEcuHighTemp)
