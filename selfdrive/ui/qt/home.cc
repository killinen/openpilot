...
void HomeWindow::offroadTransition(bool offroad) {
  ...
  sidebar->setVisible(offroad || params.getBool("Sidebar") || frogpilot_toggles.value("debug_mode").toBool());
  if (offroad) {
...
void OffroadHome::refresh() {
  ...
  version->setText(getBrand() + " v" + getVersion().left(14).trimmed() + " - " + processModelName(frogpilotUIState()->frogpilot_scene.frogpilot_toggles.value("model_name").toString()));
}
