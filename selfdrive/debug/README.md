...
bus, so the script reads `0x025` from vehicle bus 1 by default rather than expecting it to appear
as a receive frame on bus 2. Override that selection when needed, for example:

```bash
python3 selfdrive/debug/hrr_angle_calibrate.py --bus 2 --reference-bus 1
```
