...
MATRIX_Q = 20
MIN_SAMPLES = 100
# A +/-120 degree guided sweep gives 240 degrees of steering travel. Do not
# require every endpoint sample to survive the robust/low-torsion filtering.
MIN_REFERENCE_SPAN_DEG = 200.0
MIN_PHASE_SPAN_DEG = 270.0
MIN_DIRECTION_TRAVEL_DEG = 90.0
RESOLVER_REVOLUTION_DEG = 360.0
# Measured HRR geometry: one physical 360-degree resolver revolution is 22.5
# degrees of steering-shaft rotation. RMS/covariance vectors identify phase
# modulo 180 degrees, so one resolver revolution produces 180 degrees of the
# unwrapped calibration phase used below.
...
# 0x025 is quantized to 1.5 degrees, so its sample-to-sample derivative is not
# suitable for rejecting motion. Bound motion with the much smoother resolver
# phase instead; 150 deg/s is still far faster than the guided manual sweep.
MAX_STEERING_RATE_DEG_S = 150.0
MAX_RESOLVER_PHASE_RATE_DEG_S = MAX_STEERING_RATE_DEG_S * NOMINAL_PHASE_PER_STEER
MAX_FIT_RMS_DEG = 1.5
MAX_FIT_ERROR_DEG = 5.0
# IN and OU observe opposite sides of the steering torsion bar. Fit OU only
# from samples where their corrected angles are close to the unloaded
# relationship; otherwise steering effort is learned as resolver non-linearity.
# IN is the steering-reference side and is refit from all robust samples.
...
class CalibrationFit:
  ...
  def ready(self) -> bool:
    return (self.samples >= MIN_SAMPLES and self.reference_span_deg >= MIN_REFERENCE_SPAN_DEG and
            ...
            self.rms_error_deg <= MAX_FIT_RMS_DEG and self.max_error_deg <= MAX_FIT_ERROR_DEG and
            self.ou_rms_error_deg <= MAX_FIT_RMS_DEG and self.ou_max_error_deg <= MAX_FIT_ERROR_DEG)
  ...
  def format(self) -> str:
    return (f"low_torsion={self.samples}/{self.total_samples} " +
            ...
            f"OUbins={self.occupied_bins}/18 ratio={self.phase_per_steer:+.6f} " +
...
def fit_calibration_aligned(samples: list[CalibrationSample], reference_delay_s: float = 0.0) -> CalibrationFit:
  ...
  if len(low_torsion_indices) < MIN_SAMPLES:
    raise ValueError(
      ...
      f"{MAX_TORSION_DEVIATION_DEG:.1f}deg); keep hand effort light and relax briefly at a few more angles"
    )

  # IN is the steering-reference side, so use all robust samples for its final
  # ratio and ellipse correction. Steering effort should not be thrown away
  # from the most important absolute-angle fit.
  ratio, in_indices = robust_phase_ratio(samples)
  in_matrix, in_indices = solve_matrix(samples, in_indices, ratio, "in")

  # OU is across the torsion bar. Fit it only where the preliminary IN/OU
  # relationship is close to unloaded so the final OU-IN difference remains a
  # useful physical torsion-twist measurement instead of calibration bias.
  low_torsion_samples = [samples[index] for index in low_torsion_indices]
  ou_matrix, ou_indices = solve_matrix(low_torsion_samples,
                                       list(range(len(low_torsion_samples))), ratio, "ou")
  ...
  for index in ou_indices:
    sample = low_torsion_samples[index]
    target = (ratio * sample.reference_deg) % 180.0
    in_phase = corrected_phase(sample, in_matrix, "in")
    ...
    ou_bins.add(int(target // 10.0) % 18)

  # Steering-span/travel requirements describe the IN/reference sweep and do
  # not need to be re-proven by the low-torsion subset. OU instead has its own
  # electrical-phase-bin requirement above, which is the relevant coverage for
  # its 2x2 correction matrix.
  references = [samples[i].reference_deg for i in in_indices]
  phases = [samples[i].unwrapped_phase_deg for i in in_indices]
  ...
  return CalibrationFit(in_matrix + ou_matrix, ratio, len(ou_indices), len(samples),
...
def fit_calibration(samples: list[CalibrationSample],
  ...
  for _, delay_s in scored_delays:
    ...
    candidate_score = max(candidate.rms_error_deg / MAX_FIT_RMS_DEG,
                          ...
                          candidate.ou_rms_error_deg / MAX_FIT_RMS_DEG,
                          candidate.ou_max_error_deg / MAX_FIT_ERROR_DEG)
...
class HrrCalibrationSession:
  ...
  def _collect_vector(self, vector: ResolverVector, now: float) -> None:
    ...
    if self.previous_sample_at is not None:
      elapsed = now - self.previous_sample_at
      if elapsed <= 0 or abs(phase_delta) / elapsed > MAX_RESOLVER_PHASE_RATE_DEG_S:
        return
...
def fit_values(fit: CalibrationFit) -> tuple[int, ...]:
  # CAL2 has one RMS/max metric pair, so publish the worse resolver rather than
  # allowing a good IN fit to hide a poor OU fit (or vice versa).
  rms_tenths = min(255, round(max(fit.rms_error_deg, fit.ou_rms_error_deg) * 10.0))
  max_tenths = min(255, round(max(fit.max_error_deg, fit.ou_max_error_deg) * 10.0))
...
def run_guided(session: HrrCalibrationSession, timeout: float, assume_yes: bool) -> bool:
  print("\nSecure the stationary vehicle. HRR relays must be open and torque output interlocked.")
  print("Move the wheel manually and slowly: center -> about 120 deg left -> about 120 deg right -> center.")
  print("Mechanical locks are not required; the target sweep is roughly +/-120 degrees around center.")
  print("Keep hand effort light. Brief relaxed pauses at several angles improve the OU zero/torsion calibration.")
  print("You do not need to stop at every angle; low-effort portions of the sweep also count.")
  if not assume_yes:
...
def run_self_test() -> None:
  ...
  # Match the guided procedure: center -> -120 -> +120 -> center. At the
  # measured 22.5 shaft-deg/resolver-revolution ratio this already traverses
  # more than ten complete physical resolver revolutions from end to end.
  synthetic: list[CalibrationSample] = []
  unwrapped = 0.0
  previous: float | None = None
  references = ([0.0 - index * 1.5 for index in range(81)] +
                [-120.0 + index * 1.5 for index in range(1, 161)] +
                [120.0 - index * 1.5 for index in range(1, 81)])
  ...
  assert fit.ou_rms_error_deg < 0.2
  ...
  # Periodic relaxed samples span the electrical cycle and must be selected for
  # the OU fit instead of teaching torsion-bar deflection to its matrix. The IN
  # fit, however, should retain the much larger robust sample set.
  ...
  assert torsion_fit.ou_rms_error_deg < 0.2, torsion_fit.format()

  # A poor OU result must prevent READY and must be represented in the single
  # firmware fit-metric pair rather than being hidden by a good IN result.
  bad_ou_fit = CalibrationFit(fit.matrices, fit.phase_per_steer, fit.samples, fit.total_samples,
                              fit.rms_error_deg, fit.max_error_deg,
                              MAX_FIT_RMS_DEG + 0.1, MAX_FIT_ERROR_DEG + 0.1,
                              fit.torsion_rms_deg, fit.torsion_max_deg,
                              fit.reference_span_deg, fit.phase_span_deg,
                              fit.positive_travel_deg, fit.negative_travel_deg,
                              fit.occupied_bins, fit.reference_delay_s)
  assert not bad_ou_fit.ready
  bad_metrics = fit_values(bad_ou_fit)[-1]
  assert ((bad_metrics >> 16) & 0xFF) == round((MAX_FIT_RMS_DEG + 0.1) * 10.0)
  assert ((bad_metrics >> 24) & 0xFF) == round((MAX_FIT_ERROR_DEG + 0.1) * 10.0)
