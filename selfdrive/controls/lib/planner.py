...
_A_CRUISE_MIN_V  = [-1.5, -1.5, -1.5, -1.5, -1.5]
#_A_CRUISE_MIN_V  = [-1.5, -1.2, -1, -.67, -.67]
#_A_CRUISE_MIN_V  = [-1.0, -.8, -.67, -.5, -.30]
_A_CRUISE_MIN_V_NF  = [-.5, -.5, -.5, -.5, -.5]
...
_A_CRUISE_MAX_V_FOLLOWING = [1.2, 1.2, 0.65, .4]
...
def calc_cruise_accel_limits(v_ego, following, lead_following):
  ...
  if following:
    ...
    a_cruise_min = interp(v_ego, _A_CRUISE_MIN_BP, _A_CRUISE_MIN_V_NF)
...
def limit_accel_in_turns(v_ego, angle_steers, a_target, following, CP):
  ...
  # Test this to make car accelerate in turns if not following
  if following:
...
class Planner():
  ...
  def update(self, sm, pm, CP, VM, PP):
    ...
    if len(sm['model'].path.poly):
      ...
      model_speed = max(40.0 * CV.MPH_TO_MS, model_speed) # Don't slow down below 20mph
    else:
      model_speed = MAX_SPEED

    # Calculate speed for normal cruise control
    if enabled and not self.first_loop:
      accel_limits = [float(x) for x in calc_cruise_accel_limits(v_ego, following, lead_following)]
