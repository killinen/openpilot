...
class TestHrrUpdaterSafety(unittest.TestCase):
  def setUp(self):
    self.safety = libpanda_py.libpanda
    self.assertEqual(0, self.safety.set_safety_hooks(Panda.SAFETY_HRR_UPDATER, 0))
    self.safety.init_tests()

  def tx(self, address: int, data: bytes, bus: int = 2) -> bool:
  ...
  def test_only_exact_ids_bus_and_length(self):
    ...
    self.assertFalse(self.tx(0x60A, enter_boot(), bus=1))
